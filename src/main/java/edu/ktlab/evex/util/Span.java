...
public class Span implements Comparable<Span> {
	 ...
	 * the text, or the first element past the end of the span.
	 * 
	 * @return the end of a span.
	 **/
	public int getEnd() {
	 ...
	 * index with the value of end is considered outside the span.
	 * 
	 * @param index
	...
	public boolean equals(Object o) {
		...
		if (o == this) {
			...
			result = (getStart() == s.getStart())
					&& (getEnd() == s.getEnd())
					&& (getType() != null ? type.equals(s.getType()) : true)
					&& (s.getType() != null ? s.getType().equals(getType())
							: true);
		} else {
			result = false;
		}
	...
	public static String getStringAnnotated(TupleToken tuple) {
		...
		for (String label : annotation.keySet())
			for (int si = 0, sl = annotation.get(label).length; si < sl; si++) {
				...
				chunks[annotation.get(label)[si].getEnd() - 1] = chunks[annotation
						.get(label)[si].getEnd() - 1]
						+ "</"
						+ label
						+ ":"
						+ annotation.get(label)[si].getType() + ">";

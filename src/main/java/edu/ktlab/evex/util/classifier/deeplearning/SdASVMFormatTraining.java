...
public class SdASVMFormatTraining {
	static String fileTraining = "mnist";
	static String fileModel = "mnist.sda.model";
	...
	static void trainDBN() throws Exception {
		...
		int n_outs = numLabel;
		// DoubleMatrix2D X = new SparseDoubleMatrix2D(svm_vectors.size(),
		// numFeatures);
		...
		for (int i = 0; i < svm_vectors.size(); i++) {
			...
			// X.set(i, key - 1,
			// svm_vectors.get(i).getFeaturevalues().get(key));
			// }
		}
		...
		System.out.print("Construct SDA......");
		SdA sda = new SdA(train_N, n_ins, hidden_layer_sizes, n_outs, n_layers, rng);
		System.out.println("completed.");

		System.out.print("Pretrain......");
		sda.pretrain(svm_vectors, pretrain_lr, corruption_level, pretraining_epochs);
		System.out.println("completed.");

		System.out.print("Finetune......");
		sda.finetune(svm_vectors, Y, finetune_lr, finetune_epochs);
		System.out.println("completed.");

		sda.saveModel(fileModel);

		// test
		double[][] test_Y = new double[svm_vectors.size()][n_outs];
		for (int i = 0; i < svm_vectors.size(); i++) {
			sda.predict(svm_vectors.get(i), test_Y[i]);
